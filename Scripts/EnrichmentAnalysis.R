...
## Membranes protein proportion

terms <- "cell surface|plasma membrane|transmembrane|junction|extracellular|adhesion|receptor"

# Line A549
membrane_A <- grep(terms, ego_A@result$Description, 
                   value = TRUE, ignore.case = TRUE)
membrane_A <- ego_A@result[ego_A@result$Description %in% membrane_A, ]
membrane_A <- unique(unlist(strsplit(membrane_A$geneID, split = "/")))
length(membrane_A)/4814

#Line H2009
membrane_H <- grep(terms, ego_H@result$Description, 
                   value = TRUE, ignore.case = TRUE)
membrane_H <- ego_H@result[ego_H@result$Description %in% membrane_H, ]
membrane_H <- unique(unlist(strsplit(membrane_H$geneID, split = "/")))
length(membrane_H)/4951
...
# Sort protein list (required for GSEA)
