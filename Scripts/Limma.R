...
# Define up- and down-regulated genes (|log2FC| > 1 and p < 0.05)
...
# Filter to retain only significant DEGs
dep.annotated_A.DEGs <- dep.annotated_A[dep.annotated_A$DEG.Status != "Not significant", ]
dep.annotated_H.DEGs <- dep.annotated_H[dep.annotated_H$DEG.Status != "Not significant", ]

# Write DEG tables to files
write.table(dep.annotated_A.DEGs, file = "DEP_A549_annotated.csv", 
            sep = "\t", row.names = FALSE, quote = FALSE)

write.table(dep.annotated_H.DEGs, file = "DEP_H2009_annotated.csv", 
            sep = "\t", row.names = FALSE, quote = FALSE)
...
meta_DEG_A <- read_delim("MetaboAnalyst/DEG_LineaA_70Missings_FC2.csv")
meta_DEG_H <- read_delim("MetaboAnalyst/DEG_LineaH_70Missings_FC2.csv")

meta_DEG_A <- as.vector(meta_DEG_A[,1])$...1
meta_DEG_H <- as.vector(meta_DEG_H[,1])$...1
